<div align= "center">
    <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=180&text=Onejoon's%20world&animation=&fontColor=ffffff&fontSize=50" />
</div>

<div align= "center">
    <h2 style="border-bottom: 1px solid #21262d; color: #c9d1d9;"> 🛠️ Skills </h2> <br> 
    <div style="margin: 0 auto; text-align: center;"> 
        <img src="https://img.shields.io/badge/c%23-%23239120.svg?style=for-the-badge&logo=csharp&logoColor=white)">
        <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=MongoDB&logoColor=white">
    </div>
</div>

---

### 🧪 Personal Projects

#### [Front](https://github.com/your-id/wmiot-g01-public) / [Back](https://github.com/your-id/wmiot-g01-public)

<img src="https://img.shields.io/badge/nestjs-%23E0234E.svg?logo=nestjs&logoColor=white">
<img src="https://img.shields.io/badge/Next-black?logo=next.js&logoColor=white">
<img src="https://img.shields.io/badge/react-%2320232a.svg?logo=react&logoColor=%2361DAFB">
<img src="https://img.shields.io/badge/typescript-%23007ACC.svg?logo=typescript&logoColor=white">

대기환경보전법에 따른 산업용 IoT Gateway 개발 프로젝트. 실시간 환경 데이터 수집, 한국환경공단 그린링크 연동, 원격 제어 및 운영 자동화 구현.

---

### 💼 Commercial Projects

#### [wmiot_G01](https://github.com/your-id/wmiot-g01-public)

<img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/Node.js-339933?logo=nodedotjs&logoColor=white" />
<img src="https://img.shields.io/badge/MariaDB-003545?logo=mariadb&logoColor=white" />
<img src="https://img.shields.io/badge/Linux-FCC624?logo=Linux&logoColor=white">

대기환경보전법에 따른 산업용 IoT Gateway 개발 프로젝트. 실시간 환경 데이터 수집, 한국환경공단 그린링크 연동, 원격 제어 및 운영 자동화 구현.


#### [ZipsAI](https://github.com/your-id/wallpad-public)

<img src="https://img.shields.io/badge/Node.js-339933?logo=nodedotjs&logoColor=white" />
<img src="https://img.shields.io/badge/SpringBoot-6DB33F?logo=springboot&logoColor=white" />
<img src="https://img.shields.io/badge/Flutter-02569B?logo=flutter&logoColor=white" />
<img src="https://img.shields.io/badge/MariaDB-003545?logo=mariadb&logoColor=white" />
<img src="https://img.shields.io/badge/InfluxDB-22ADF6?logo=InfluxDB&logoColor=white">

월패드 실시간 모니터링 및 원격 제어 시스템 구축 프로젝트. 위치 데이터 수집 시스템 설계 및 서버 트래픽 최적화 수행.

    
    
